...
@SpringBootTest(classes = SecureWebClientTest.TestApplication.class,
...
class SecureWebClientTest {
	...
	@Test
	void shouldExchangeSecureEnvelopeWithWebClient() {
		WebClient client = WebClient.builder()
			.baseUrl("http://localhost:" + this.port)
			.filter(new SecureWebClientExchangeFilterFunction(this.operations))
			.build();

		OrderResponse response = client.post()
			.uri("/orders")
			.bodyValue(new OrderRequest("TEST-WEBCLIENT-1", new BigDecimal("10.00")))
			.retrieve()
			.bodyToMono(OrderResponse.class)
			.block();

		Assertions.assertThat(response.getOrderNo()).isEqualTo("TEST-WEBCLIENT-1");
		Assertions.assertThat(response.getStatus()).isEqualTo("ACCEPTED");
		Assertions.assertThat(this.controller.getLastRequest().getOrderNo()).isEqualTo("TEST-WEBCLIENT-1");
		Assertions.assertThat(this.controller.getLastRequest().getAmount()).isEqualByComparingTo("10.00");
	}
