...
@SpringBootTest(classes = SecureFeignClientTest.TestApplication.class,
		webEnvironment = SpringBootTest.WebEnvironment.RANDOM_PORT)
class SecureFeignClientTest {
	...
	@Test
	void shouldExchangeSecureEnvelopeWithFeign() {
		TestOrderClient client = Feign.builder()
			.encoder(new SecureFeignEncoder(new JsonEncoder(this.objectMapper), this.operations))
			.decoder(new SecureFeignDecoder(new JsonDecoder(this.objectMapper), this.operations))
			.target(TestOrderClient.class, "http://localhost:" + this.port);

		OrderResponse response = client.create(new OrderRequest("TEST-FEIGN-1", new BigDecimal("8.00")));

		Assertions.assertThat(response.getOrderNo()).isEqualTo("TEST-FEIGN-1");
		Assertions.assertThat(response.getStatus()).isEqualTo("ACCEPTED");
		Assertions.assertThat(this.controller.getLastRequest().getOrderNo()).isEqualTo("TEST-FEIGN-1");
		Assertions.assertThat(this.controller.getLastRequest().getAmount()).isEqualByComparingTo("8.00");
	}

	interface TestOrderClient {
	...
	static class JsonEncoder implements Encoder {
	...
	static class JsonDecoder implements Decoder {
		...
		JsonDecoder(ObjectMapper objectMapper) {
			this.objectMapper = objectMapper;
		}

		@Override
		public Object decode(Response response, Type type) throws IOException {
			return this.objectMapper.readValue(Util.toByteArray(response.body().asInputStream()),
