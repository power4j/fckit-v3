...
本示例演示当前阶段 Feign 注解式 Body 加密处理的技术验证效果。

示例要点：

1. Feign 接口方法使用 `@SecureExchange("body-strict-v1")` 标注策略。
2. 示例内 `DemoSecureFeignEncoder` 读取 Feign 方法元数据，将原始 JSON Body 加密签名为 SDE request envelope。
...
5. 示例内 `DemoSecureFeignDecoder` 验签、重放校验并解密响应，再交给调用方业务对象。
...
- 这里的 Encoder / Decoder 只放在示例模块内，不属于正式 Feign 交付代码。
